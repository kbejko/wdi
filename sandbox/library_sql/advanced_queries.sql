...
SELECT * FROM books JOIN authors ON authors.id = books.author_id WHERE books.publication_date > 1980 AND authors.nationality <> 'United States of America';
-- Find all authors whose names start with 'J'.
SELECT * FROM authors WHERE name LIKE 'J%';
-- Find all books whose titles contain 'the'.
SELECT * FROM books WHERE title ILIKE '%the%';
-- Find all authors who have written books with that start with the letter 'N'.
SELECT * FROM books JOIN authors ON authors.id = books.author_id WHERE books.title LIKE 'N%';
